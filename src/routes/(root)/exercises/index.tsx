...
export const Route = createFileRoute('/(root)/exercises/')({
...
function RouteComponent() {
  ...
  const deleteMutation = useRemoveExercise();
  const { isMobile } = useMediaQuery();

  const [formOpen, setFormOpen] = useState(false);
  const [editingEntry, setEditingEntry] = useState<ExerciseDto | null>(null);
  const [deleteId, setDeleteId] = useState<number | null>(null);

  const handleSubmit = (formData: CreateExerciseInput) => {
    if (editingEntry) {
  ...
  const openEdit = (entry: ExerciseDto) => {
      ...
      <Card>
        ...
        <CardContent>
          <ArrayDataWrapper
            ...
            isLoading={isLoading}
            error={error}
            emptyMessage='No exercises defined yet.'
          >
            {(items) =>
              isMobile ? (
...
function DesktopTable({
  ...
  items: ExerciseDto[];
  onEdit: (item: ExerciseDto) => void;
  onDelete: (id: number) => void;
}) {
  return (
    <div className='overflow-x-auto'>
      <div className='grid grid-cols-[2fr_1fr_1fr_auto] gap-x-4 min-w-[480px]'>
        <div className='grid grid-cols-subgrid col-span-full text-xs font-medium text-muted-foreground uppercase bg-muted/50 rounded-t-lg'>
          ...
          <div className='px-3 py-2'>Cal/min</div>
        ...
        {items.map((item) => (
          ...
          >
            <div className='px-3 font-medium'>{item.name}</div>
            <div className='px-3 text-muted-foreground capitalize'>
              {item.exerciseType.charAt(0) + item.exerciseType.slice(1).toLowerCase()}
            </div>
            <div className='px-3 text-muted-foreground'>
              {item.caloriesPerMin != null ? item.caloriesPerMin : '—'}
            </div>
    ...
    <div className='flex flex-col gap-3'>
      {items.map((item) => (
        <div key={item.id} className='border rounded-lg p-4 space-y-2'>
          ...
          {item.caloriesPerMin != null && (
            <p className='text-xs text-muted-foreground'>{item.caloriesPerMin} cal/min</p>
          )}
